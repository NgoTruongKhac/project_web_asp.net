...
namespace LaptopStore.Controllers
{
	public class UserInfoController : Controller
	{
		...
		public async Task<IActionResult> ViewUserInfo()
		{
			...
			List<Address> ListAddress = await connectDatabase.addresses
		...
		public async Task<IActionResult> AddAddress(Address address)
		{
			if (address.IsDefault)
			{
				...
				var userAddresses = await connectDatabase.addresses.Where(a => a.UserId == address.UserId).ToListAsync();
			...
			await connectDatabase.addresses.AddAsync(address);
			await connectDatabase.SaveChangesAsync();
		...
		{
			var address = await connectDatabase.addresses.FirstOrDefaultAsync(a => a.AddressId == addressId);

			if (address != null)
			{
				connectDatabase.addresses.Remove(address);
				await connectDatabase.SaveChangesAsync();
			}
